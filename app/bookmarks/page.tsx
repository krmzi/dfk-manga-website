...
const FavCard = ({ manga }: { manga: any }) => (
    <Link href={`/manga/${manga.id}`} className="group relative block aspect-[2/3] rounded-xl overflow-hidden border border-white/10 hover:border-red-600 hover:-translate-y-1 transition-all duration-300">
        <img 
    ...
    const fetchBookmarks = async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(`
            manga_id,
            mangas (
                id, title, cover_image, status, rating
            )
        `)
        .eq('user_id', user.id);
