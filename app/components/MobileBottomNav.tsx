...
export default function MobileBottomNav() {
    ...
    return (
        <div className="fixed bottom-0 left-0 w-full z-[9999] md:hidden pb-[env(safe-area-inset-bottom)]">
            {/* Blur Backdrop */}
            <div className="absolute inset-0 bg-[#050505]/95 backdrop-blur-xl border-t border-white/5 shadow-[0_-10px_40px_rgba(0,0,0,0.8)]"></div>

            <div className="relative flex justify-around items-center h-[65px] px-2 pb-1">
                {navItems.map((item) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <span className={`text-[10px] font-bold ${active ? 'opacity-100' : 'opacity-0 scale-0'} transition-all duration-300 absolute bottom-1`}>
                                {item.name}
                            </span>
                        </Link>
                    );
