"use client"; 
import { useState, useMemo, useEffect, Suspense } from "react"; // إضافة useEffect
import { Search, ChevronDown, Clock, Star, Filter, X } from "lucide-react";
import Link from "next/link";
import { useSearchParams } from "next/navigation"; // إضافة هوك قراءة الرابط
...
// ... (كود BrowseCard يبقى كما هو، اختصاراً للمساحة) ...
const BrowseCard = ({ id, title, cover_image, rating, status, country, latestChNum }: any) => (
  <Link href={`/manga/${id}`} className="group flex flex-col bg-[#0f0f10] border border-white/5 rounded-2xl overflow-hidden hover:border-red-600/40 hover:-translate-y-1 transition-all duration-300 shadow-lg">
    {/* ... (نفس محتوى الكارد السابق) ... */}
...
function MangaFilterLogic({ initialMangas }: { initialMangas: any[] }) {
    const searchParams = useSearchParams(); // ✅ قراءة الرابط
    
    // الحالة الافتراضية تأتي من الرابط (إن وجد)
    ...
    // ✅ تأثير لتحديث البحث عند تغيير الرابط
    useEffect(() => {
    ...
    // ... (بقية كود DropdownFilter كما هو، اختصاراً) ...
...
// التغليف بـ Suspense ضروري لـ useSearchParams
